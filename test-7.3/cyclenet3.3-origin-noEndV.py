...
class Config:
    def __init__(self):
        ...
        # --- 模型超参数 ---
        self.meta_cycle_len = 50  # 定义一个元周期长度，比如假设电池每100个循环有一个宏观上的周期性变化
        self.sequence_length = 1 # A文件的序列长度 (弛豫段电压的点数)
        self.scalar_feature_dim = len(self.features_from_C)  # B和C文件合并后的标量特征数量
...
class CycleNetForSOH(nn.Module):
    def __init__(self, configs):
        ...
        # 1. 为两种不同的输入数据创建编码器
        self.sequence_encoder = nn.Linear(configs.sequence_length * 3, configs.d_model // 2)
...
def load_and_preprocess_data(config):
    ...
    for battery_id in sorted(list(set(all_ids))):
        try:
            ...
            # feature_cols = ['弛豫段电压1', '弛豫段电压2', '弛豫段电压3', '弛豫段电压4', '弛豫段电压5', '弛豫段电压6']
...
def main():
    ...
    if config.mode in ['both', 'validate']:
        ...
        val_results_df = pd.DataFrame(
            {'循环号': val_cycle_nums, 'True_Capacity': val_labels, 'Predicted_Capacity': val_preds})
        test_results_df = pd.DataFrame(
            {'循环号': test_cycle_nums, 'True_Capacity': test_labels, 'Predicted_Capacity': test_preds})
