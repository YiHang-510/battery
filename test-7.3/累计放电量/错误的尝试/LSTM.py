...
class Config:
    def __init__(self):
        ...
        self.save_path = '/home/scuee_user06/myh/电池/result-累计放电容量/LSTM/all'  # 保存模型、结果和图像的文件夹路径
...
class LSTMPredictionModel(nn.Module):
    ...
    def forward(self, x_seq, x_scalar):
        ...
        # --- 1. 特征编码 ---
        # 通过LSTM处理序列数据，并获取最后一个时间步的隐藏状态
        _, (h_n, _) = self.sequence_encoder(x_seq)
        seq_embedding = h_n[-1] # 形状: (batch, d_model // 2)

        # 通过全连接层处理标量数据
        scalar_embedding = self.scalar_encoder(x_scalar) # 形状: (batch, d_model // 2)
...
def main():
    ...
    model = LSTMPredictionModel(config).to(config.device)
