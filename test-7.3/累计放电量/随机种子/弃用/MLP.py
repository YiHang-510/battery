...
class Config:
    def __init__(self):
        ...
        self.save_path = '/home/scuee_user06/myh/电池/result-累计放电容量/MLP/all'  # 保存模型、结果和图像的文件夹路径
...
class MLPModel(nn.Module):
    def __init__(self, configs):
        ...
        # 1. 序列数据编码器：一个简单的线性层处理压平后的序列
        self.sequence_encoder = nn.Sequential(
            nn.Linear(flattened_seq_dim, configs.hidden_dim // 2),
            nn.ReLU(),
        ...
        self.scalar_encoder = nn.Sequential(
            nn.Linear(configs.scalar_feature_dim, configs.hidden_dim // 2),
            nn.ReLU(),
            nn.Dropout(configs.dropout)
        )

        # 3. 预测头：融合两个分支的特征并进行最终预测
        # 输入维度是两个编码器输出维度之和
        self.prediction_head = nn.Sequential(
            nn.Linear(configs.hidden_dim // 2 + configs.hidden_dim // 2, configs.ff_dim),
            nn.ReLU(),
            nn.Dropout(configs.dropout),
            nn.Linear(configs.ff_dim, 1)
        )

    def forward(self, x_seq, x_scalar, cycle_number):
        # cycle_number 在这个模型中不会被使用，但为了保持接口一致性，我们仍然接收它

        # x_seq: (batch, seq_len, feature_dim) -> (batch, seq_len * feature_dim)
        # x_scalar: (batch, scalar_dim)

        # --- 1. 压平并编码序列特征 ---
        # 使用 view(x_seq.size(0), -1) 来动态地压平
...
def main():
    ...
    for run_number in range(1, num_runs + 1):
        ...
        # =================================================================
        # === 核心修改：实例化新的 MLPModel 而不是 AttentionLSTM ===
        # =================================================================
        model = MLPModel(config).to(config.device)
        # =================================================================
