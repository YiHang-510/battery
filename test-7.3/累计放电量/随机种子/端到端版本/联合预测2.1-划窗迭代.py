...
class Config:
    ...
    save_path: str = '/home/scuee_user06/myh/电池/result-累计放电容量/result-dualnet2.1/Lwindow-monoExp/20'
...
class ExpNetMono(nn.Module):
    ...
    支持历史容量趋势嵌入：hist_emb (B, H)。
    c, q_t 均应为 shape (B,1) 的张量（已标准化）。"""

    def __init__(self, n_terms=16, out_dims=2, d_hist=0):
        super().__init__()
        ...
        # 历史容量趋势的线性调制（不破坏单调性，因为仍对 softplus 之前的基底做加法）
        self.hist_fc_c = nn.Linear(d_hist, 1) if d_hist > 0 else None
        self.hist_fc_q = nn.Linear(d_hist, 1) if (out_dims > 1 and d_hist > 0) else None

    def _phi(self, c):
        b = -(torch.nn.functional.softplus(self.log_b) + 1e-8)  # 保证 b<0
        return torch.exp(c * b.unsqueeze(0))  # (B,K)

    def forward(self, c, q_t: torch.Tensor | None = None, hist_emb: torch.Tensor | None = None):
        phi = self._phi(c)  # (B,K)
        base_c = self.Wc(phi) + self.bias_c  # (B,1)
        if hist_emb is not None and self.hist_fc_c is not None:
            base_c = base_c + self.hist_fc_c(hist_emb)
        dC = torch.nn.functional.softplus(base_c)  # ΔC >= 0
        C_next = c + dC

        if self.out_dims > 1:
            base_q = self.Wq(phi) + self.bias_q
            if hist_emb is not None and self.hist_fc_q is not None:
                base_q = base_q + self.hist_fc_q(hist_emb)
            if q_t is not None:
                dQ = -torch.nn.functional.softplus(base_q)  # ΔQ <= 0
                Q_next = q_t + dQ
            else:
                Q_next = base_q  # 绝对值回归（外部加单调惩罚）
            return torch.cat([C_next, Q_next], dim=1)
        else:
            return C_next


class CapHistEncoder(nn.Module):
    """把最近 L 个标准化后的容量序列 (B,L,1) 编码成一个向量 (B,H)。"""

    def __init__(self, d_model: int):
        super().__init__()
        H = max(16, d_model // 4)
        self.rnn = nn.GRU(input_size=1, hidden_size=H, batch_first=True)
        self.proj = nn.Linear(H, H)
        self.out_dim = H

    def forward(self, c_hist: torch.Tensor):
        # c_hist: (B,L,1)
        _, h = self.rnn(c_hist)  # (1,B,H)
        h = h.squeeze(0)  # (B,H)
...
def _make_L_windows(merged_one, cfg: Config, stride=1, randN=0):
    ...
    for t in valid_t:
        ...
        row = {
            ...
            'C_hist': df.loc[t - cfg.window_L + 1:t, cfg.col_C].values.astype(np.float32),
...
class LWindowDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        Q_t = torch.tensor(r['Q_t'], dtype=torch.float32) if ('Q_t' in r) and not pd.isna(r['Q_t']) else torch.tensor(
            0.0, dtype=torch.float32)
        Q_t1 = torch.tensor(r['Q_t1'], dtype=torch.float32) if ('Q_t1' in r) and not pd.isna(
            r['Q_t1']) else torch.tensor(0.0, dtype=torch.float32)
        C_hist = torch.from_numpy(r['C_hist']).view(-1, 1)  # (L,1) standardized

        return x_win, x_sca, cycle_t, C_t, C_t1, Q_t, Q_t1, C_hist
...
def train_one_epoch(E_net, P_net, hist_enc, loader, optimizer, cfg: Config, has_Q: bool):
    E_net.train();
    P_net.train()
    hist_enc.train()
    mse = nn.MSELoss()
    ...
    total = 0.0
    for (x_win, x_sca, cycle_t, C_t, C_t1, Q_t, Q_t1, C_hist) in loader:
        ...
        C_hist = C_hist.to(cfg.device)
        hist_emb = hist_enc(C_hist)
        C_hist = C_hist.to(cfg.device)
        hist_emb = hist_enc(C_hist)
        ...
        if has_Q and cfg.use_q_t_as_input:
            out = P_net(C_in, q_t=Q_t, hist_emb=hist_emb)
        else:
            out = P_net(C_in, q_t=None, hist_emb=hist_emb)
        C_next_hat = out[:, 0:1]
...
@torch.no_grad()
def evaluate(E_net, P_net, hist_enc, loader, cfg: Config, scalers, has_Q: bool):
    E_net.eval();
    P_net.eval();
    hist_enc.eval()
    mse = nn.MSELoss()
    ...
    for (x_win, x_sca, cycle_t, C_t, C_t1, Q_t, Q_t1, C_hist) in loader:
        ...
        out = P_net(C_t, q_t=Q_t if (has_Q and cfg.use_q_t_as_input) else None, hist_emb=hist_emb)
        C_next_hat = out[:, 0:1]
    ...
    Ct_pred = _cat(all['Ct_pred']);
    Ct_true = _cat(all['Ct_true'])
    Ct1_pred = _cat(all['Ct1_pred']);
    Ct1_true = _cat(all['Ct1_true'])
    metrics = {
    ...
    if has_Q and len(all['Qt1_pred']) > 0:
        Qt1_pred = _cat(all['Qt1_pred']);
        Qt1_true = _cat(all['Qt1_true'])
        metrics.update({
...
def plot_series_teacher_forced_for_battery(df_bat, cfg, scalers, P_net, has_Q,
    ...
    """
    【修改版】绘制 Teacher-Forcing 模式下的 Q-vs-C 退化曲线，以验证 ExpNet 性能。
    横坐标为“累计放电容量 C”，纵坐标为“容量 Q”。
    """
    ...
    plt.close()
    print(f"[SAVE] Correctly plotted Q-vs-C curve to: {out_path}")


@torch.no_grad()
def rul_by_Q(df_bat, cfg, scalers, E_net, P_net, hist_enc, soh_thr=0.8, q0=None, start_idx=None, max_h=5000,
...
def main():
    ...
    hist_enc = CapHistEncoder(cfg.d_model).to(cfg.device)
    P_net = ExpNetMono(n_terms=cfg.exp_n_terms, out_dims=2 if has_Q else 1, d_hist=hist_enc.out_dim).to(cfg.device)
    ...
    for ep in range(1, cfg.epochs + 1):
        tr_loss = train_one_epoch(E_net, P_net, hist_enc, train_loader, opt_all, cfg, has_Q)
        val_loss, val_metrics = evaluate(E_net, P_net, hist_enc, val_loader, cfg, scalers, has_Q)
        log_rows.append({'epoch': ep, 'train_loss': tr_loss, 'val_loss': val_loss,
        ...
        if val_key < best_val:
            ...
            torch.save(hist_enc.state_dict(), os.path.join(cfg.save_path, 'H_best.pth'))
        else:
            no_improve += 1
    ...
    hist_enc.load_state_dict(torch.load(os.path.join(cfg.save_path, 'H_best.pth'), map_location=cfg.device))
    # --- THIS IS THE CORRECTED LINE ---
    test_loss, test_metrics = evaluate(E_net, P_net, hist_enc, test_loader, cfg, scalers, has_Q)
    # ----------------------------------
    print("\n== Final Test ==")
    ...
    def gather_preds(loader):
        ...
        for (x_win, x_sca, cycle_t, C_t, C_t1, Q_t, Q_t1, C_hist) in loader:
            ...
            C_true.append(scalers['C'].inverse_transform(C_t1.cpu().numpy()).ravel());
            C_pred.append(C_next)
            if has_Q and out.size(1) > 1:
                Q_next = scalers['Q'].inverse_transform(out[:, 1:2].cpu().numpy()).ravel()
                Q_true.append(scalers['Q'].inverse_transform(Q_t1.cpu().numpy()).ravel());
