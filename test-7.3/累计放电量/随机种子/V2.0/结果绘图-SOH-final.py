...
out_dir = r"D:\任务归档\电池\研究\二稿-小论文1号\DOCUMENT\fig\Results\SOH_estimation_final"
...
def save_one_small_fig(title, savename, x, real, est, color,
    ...
    # Only start plotting once true SOH drops below 0.8
    mask = real < 0.8
    if mask.any():
        x = x[mask]
        real = real[mask]
        est = est[mask]
    if len(x) > 0:
        # Drop the last point so it is not drawn
        x = x[:-1]
        real = real[:-1]
        est = est[:-1]
