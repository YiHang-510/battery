...
def save_one_small_fig(title, savename, x, real, est, color,
    ...
    # ----------------------
    # 上图：SOH vs Cycle (已修改 X 轴)
    # ----------------------
    ax1 = fig.add_subplot(gs[0, 0])
    ax1.plot(x, real, ls='none', marker='o', ms=1, color=c_real, label='Real SOH', alpha=0.8)
    ax1.plot(x, est, ls='none', marker='D', ms=1, color=color, label='Estimated SOH', alpha=0.8)
    ...
    # ----------------------
    # 下图：Predicted vs Real (对角图保持不变)
    # ----------------------
    ax2 = fig.add_subplot(gs[1, 0])

    # 1. 对角线
    plot_min, plot_max = limit_min, limit_max
    ax2.plot([plot_min, plot_max], [plot_min, plot_max], 'k--', lw=1, label='Perfect Prediction')

    # 2. 散点
    ax2.scatter(real, est, s=8, c=color, marker='o', alpha=0.6, edgecolors='none', label='Prediction')

    # 3. 标签
    ax2.set_xlabel("Real SOH", fontsize=10)
    ax2.set_ylabel("Estimated SOH", fontsize=10)
    ...
    # ax2.text(0.05, 0.90, f"RMSE: {rmse:.4f}", transform=ax2.transAxes,
...
print("所有 SOH-Cycle 对角图已成功生成并保存。")