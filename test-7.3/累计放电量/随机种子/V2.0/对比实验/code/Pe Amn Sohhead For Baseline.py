# 文件名: PE_AMN_Adapted.py
# 描述: 
# 1. 采用 Jiang et al. (Journal of Energy Storage 112, 2025)  中的 PE-AMN 思想。
# 2. 核心模型替换为 CNN-Autoencoder + GRU (作为Mamba的替代) + PIHF (标量特征) 融合。
# 3. Autoencoder [cite: 181, 215] 用于处理序列数据 (x_seq)。
# 4. PIHF Encoder (MLP) 用于处理标量数据 (x_scalar) [cite: 182, 183]。
# 5. GRU (替代Mamba [cite: 215]) 对融合后的特征进行时序建模。
# 6. SOH_Head [cite: 305] 和 Q_Head (自定义) 用于最终预测。
# 7. 训练/评估/保存框架与 TimeMixer+PI-Res-V2.0.py 保持一致。
...
class Config:
    def __init__(self):
        ...
        self.save_path = '/home/scuee_user06/myh/电池/result-累计放电容量V2.0/PE_AMN_Adapted/20' # <-- 修改了保存路径
        ...
        self.sequence_feature_dim = 7  # A文件的输入特征维度 (x_seq)
        self.sequence_length = 1      # 序列长度
        self.scalar_feature_dim = len(self.features_from_C) # C文件的输入特征维度 (x_scalar)

        # --- 模型超参数 (PE-AMN Adapted) ---
        # 对应文献 [cite: 215, 232]
        self.ae_input_channels = self.sequence_feature_dim
        self.ae_latent_dim = 32      # Autoencoder 提取的特征维度 Z [cite: 218]
        self.ae_hidden_channels = 16 # Autoencoder 中间层
        
        # 对应文献 [cite: 183] (PIHFs 编码器)
        self.pihf_embed_dim = 16     # 标量特征(PIHF替代品)的编码维度

        # 对应文献 [cite: 215] (Mamba 替代品)
        self.mamba_hidden_dim = 64   # GRU (Mamba替代品) 的隐藏层维度
        self.mamba_layers = 2        # GRU 的层数
        self.dropout = 0.1
        
        # 对应文献 [cite: 173, 227] (Autoencoder 重构损失)
        self.recon_loss_weight = 0.1 # 重构损失的权重
        ...
        self.seed = 2025
        self.mode = 'both'
        self.task_weights = {"q": 1.0, "soh": 100000}

        # --- 设备设置 (与原脚本一致) ---
        self.use_gpu = True
        self.device = torch.device("cuda" if self.use_gpu and torch.cuda.is_available() else "cpu")
        self.use_amp = True # 启用AMP
...
# --- 3. 新的预测网络 (PE_AMN_Adapted) ---

# 3.1 CNN 自动编码器 (基于文献 [cite: 181, 215, 232])
class CnnAutoencoder(nn.Module):
    def __init__(self, input_channels, hidden_channels, latent_dim, dropout):
        super().__init__()
        # 编码器 [cite: 218]
        # (B, C_in, L_in) -> (B, D_latent, L_out)
        # 注意: L_in=1, K=3, P=1, L_out=1
        ...
        # 解码器 [cite: 218]
        # (B, D_latent, L_out) -> (B, C_in, L_in)
        self.decoder = nn.Sequential(
            nn.ConvTranspose1d(latent_dim, hidden_channels, kernel_size=3, padding=1),
            nn.BatchNorm1d(hidden_channels),
            nn.ReLU(True),
            nn.Dropout(dropout),
...
# 3.2 物理增强的 AMN 适配模型 (PE_AMN_Adapted)
class PE_AMN_Adapted(nn.Module):
    def __init__(self, cfg: Config):
        super().__init__()
        self.cfg = cfg

        # (1) CNN 自动编码器 [cite: 215, 232]
        # 用于从 x_seq (替代 ICA/DTA ) 中提取特征 Z
        self.autoencoder = CnnAutoencoder(
            input_channels=cfg.ae_input_channels,
            hidden_channels=cfg.ae_hidden_channels,
            latent_dim=cfg.ae_latent_dim,
            dropout=cfg.dropout
        )

        # (2) PIHF 编码器 [cite: 182, 183]
        # 用于从 x_scalar (替代 PIHFs) 中编码
        self.pihf_encoder = nn.Sequential(
            nn.Linear(cfg.scalar_feature_dim, cfg.pihf_embed_dim),
            nn.ReLU(),
            nn.Dropout(cfg.dropout)
        )
        
        # (3) Mamba 模块 (使用 GRU 替代) [cite: 215]
        # GRU/Mamba 用于处理时序特征
        # 融合 Z (ae_latent_dim) 和 PIHFs (pihf_embed_dim) 作为输入
        self.mamba_input_dim = cfg.ae_latent_dim + cfg.pihf_embed_dim
        self.mamba_block = nn.GRU(
            input_size=self.mamba_input_dim,
            hidden_size=cfg.mamba_hidden_dim,
            num_layers=cfg.mamba_layers,
            batch_first=True, # 输入形状 (B, L, D_in)
            dropout=cfg.dropout if cfg.mamba_layers > 1 else 0
        )

        # (4) SOH 预测头 [cite: 305]
        # (对应文献中的 AMN_soh)
        self.soh_head = nn.Sequential(
            nn.Linear(cfg.mamba_hidden_dim, cfg.mamba_hidden_dim // 2),
            nn.ReLU(),
            nn.Dropout(cfg.dropout),
            nn.Linear(cfg.mamba_hidden_dim // 2, 1),
            nn.Sigmoid() # SOH 输出约束到 [0, 1]
        )

        # (5) Q (累计容量) 预测头 (自定义)
        # 遵循文献的联合思想 ，使用 SOH 预测值和 t_norm 作为辅助输入
        self.q_head_input_dim = cfg.mamba_hidden_dim + 1 + 1  # 1 for SOH, 1 for t_norm
        self.q_head = nn.Sequential(
            nn.Linear(self.q_head_input_dim, cfg.mamba_hidden_dim),
            nn.ReLU(),
            nn.Dropout(cfg.dropout),
            nn.Linear(cfg.mamba_hidden_dim, 1)
            # Q 是累积值，不加激活
        )

    def forward(self, x_seq, x_scalar, t_norm):
        # x_seq:   (B, L, C_in) = (B, 1, 7)
        # x_scalar: (B, D_scalar) = (B, 2)
        # t_norm:   (B,)
        
        # --- (A) 特征提取 (Autoencoder) ---
        # 调整维度以适应 Conv1d: (B, L, C_in) -> (B, C_in, L)
        x_seq_t = x_seq.transpose(1, 2) # (B, 7, 1)
        
        # 提取特征 Z 并获取重构 x_recon [cite: 218]
        # z: (B, D_latent, L_latent) = (B, 32, 1)
        # x_recon: (B, C_in, L) = (B, 7, 1)
        z, x_recon = self.autoencoder(x_seq_t)

        # --- (B) 特征融合 (Physics Enhancement) [cite: 183, 304] ---
        
        # 1. 准备序列特征 Z
        # (B, D_latent, L_latent) -> (B, L_latent, D_latent)
        z_seq = z.transpose(1, 2) # (B, 1, 32)
        
        # 2. 准备 PIHF (标量) 特征
        # (B, D_scalar) -> (B, D_pihf)
        h_pihf = self.pihf_encoder(x_scalar) # (B, 16)
        
        # 3. 扩展 PIHF 特征以匹配序列长度
        # (B, D_pihf) -> (B, 1, D_pihf)
        B, L, _ = z_seq.shape
        h_pihf_expanded = h_pihf.unsqueeze(1).expand(-1, L, -1) # (B, 1, 16)

        # 4. 融合 (Concat) [cite: 304]
        # (B, L, D_latent) + (B, L, D_pihf) -> (B, L, D_mamba_in)
        fused_seq = torch.cat([z_seq, h_pihf_expanded], dim=2) # (B, 1, 32+16=48)

        # --- (C) Mamba (GRU) 时序建模 [cite: 215] ---
        # mamba_out: (B, L, D_mamba) = (B, 1, 64)
        mamba_out, _ = self.mamba_block(fused_seq)
        
        # 提取最后一个时间步 (或 L=1 时的) 特征
        h_final = mamba_out.mean(dim=1) # (B, 64)
        
        # --- (D) 预测头 ---
        
        # 1. SOH 预测 [cite: 305]
        pred_s = self.soh_head(h_final).squeeze(-1) # (B,)
        
        # 2. Q (累计容量) 预测
        # 准备输入: h_final, pred_s, t_norm
        pred_s_input = pred_s.unsqueeze(1) # (B, 1)
        t_norm_input = t_norm.unsqueeze(1) # (B, 1)
        
        q_head_input = torch.cat([
            h_final,          # (B, 64)
            pred_s_input,     # (B, 1) - 遵循联合估算思想 
            t_norm_input      # (B, 1) - 预测累积值所需的时间特征
        ], dim=1)
        
        pred_q = self.q_head(q_head_input).squeeze(-1) # (B,)

        # 返回预测值和重构值 (用于计算损失)
        return (pred_q, pred_s), x_recon, x_seq_t
...
def load_and_preprocess_data(config):
    ...
    # 为 Q 目标创建一个归一化器
    scaler_q_target = StandardScaler()
    # 关键: 仅在训练数据上 'fit'
    scaler_q_target.fit(train_df[[target_q_col]])
    ...
    df.loc[:, [target_q_col]] = scaler_q_target.transform(df[[target_q_col]])
    ...
    scalers = {
        ...
        'q_target': scaler_q_target  # 保存这个归一化器
    }
...
def train_epoch(model, dataloader, optimizer, criterion, config):
    ...
    for batch in dataloader:
        ...
        with autocast(dtype=amp_dtype, enabled=use_amp):
            # --- 修改: 接收重构值 ---
            # (outputs, aux) = model(...)  <- 旧的
            (outputs, x_recon, x_seq_t) = model(batch_seq, batch_scalar, t_norm)
            pred_q, pred_s = outputs

            # --- 修改: 增加重构损失 [cite: 173, 227] ---
            loss_q   = criterion(pred_q, batch_y_q)
            loss_soh = criterion(pred_s, batch_y_s)
            loss_recon = criterion(x_recon, x_seq_t) # Autoencoder 损失
            
            # --- 修改: 移除 loss_reg，替换为 loss_recon ---
            # loss_reg = ... (aux.get("c_h", 0.0) ** 2).mean() # <- 旧的
...
def evaluate(model, dataloader, criterion, device, config):
    ...
    with torch.no_grad():
        for batch in dataloader:
            ...
            loss_q   = criterion(pred_q, batch_y_q)
            loss_soh = criterion(pred_s, batch_y_s)
            loss_recon = criterion(x_recon, x_seq_t)
...
def plot_soh_results(labels, preds, title, save_path):
    ...
    plt.ylabel('SOH (State of Health)', fontsize=12)
    plt.legend()
    plt.grid(True)
    plt.ylim(min(0.7, np.min(labels) * 0.95), max(1.0, np.max(labels) * 1.0))
...
def main():
    ...
    for run_number in range(1, num_runs + 1):
        ...
        # --- 修改: 初始化 PE_AMN_Adapted 模型 ---
        # 移除旧的 tm_config
        model = PE_AMN_Adapted(config).to(config.device)
        # --- 修改结束 ---

        criterion = nn.MSELoss()
        optimizer = optim.Adam(model.parameters(), lr=config.learning_rate, weight_decay=config.weight_decay)
        # GradScaler 在 train_epoch 内部处理
        ...
        if config.mode in ['both', 'validate']:
            ...
            scaler_q_target = scalers.get('q_target')
            if scaler_q_target is None:
                print("警告: 找不到 'q_target' scaler，指标将按归一化尺度计算。")

            # --- (评估和结果保存部分与原脚本完全一致，无需修改) ---
            ...
            # test_preds_orig_q = np.clip(test_preds_q, a_min=0.0, a_max=None) # <- 旧的
            # test_labels_orig_q = test_labels_q # <- 旧的

            # 对 Q 的预测和标签进行反归一化
            if scaler_q_target:
                test_preds_orig_q = scaler_q_target.inverse_transform(test_preds_q.reshape(-1, 1)).squeeze()
                test_labels_orig_q = scaler_q_target.inverse_transform(test_labels_q.reshape(-1, 1)).squeeze()
            else:
                test_preds_orig_q = test_preds_q
                test_labels_orig_q = test_labels_q

            # 在反归一化 *之后* 再进行裁剪
            test_preds_orig_q = np.clip(test_preds_orig_q, a_min=0.0, a_max=None)
            test_preds_orig_s = np.clip(test_preds_s, a_min=0.0, a_max=1.0)
        ...
        # --- (汇总保存部分与原脚本完全一致，无需修改) ---
        print(f"\n{'=' * 30} 第 {run_number}/{num_runs} 次实验完成 {'=' * 30}")
        # --- 循环结束 ---
