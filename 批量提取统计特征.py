...
def extract_features_updated(df):
    ...
    for cyc, data in df.groupby('循环号'):
        ...
        if not discharge_seg.empty:
            ...
            discharge_time = discharge_seg['总时间'].max() - discharge_seg['总时间'].min()
        else:
            discharge_area = discharge_time = 0

        # 恒流充电时间
        cc_charge_seg = data[data['工步类型'] == '恒流充电']
        cc_charge_time = cc_charge_seg['总时间'].max() - cc_charge_seg['总时间'].min() if not cc_charge_seg.empty else 0

        # 恒压充电时间
        cv_charge_seg = data[data['工步类型'] == '恒压充电']
        cv_charge_time = cv_charge_seg['总时间'].max() - cv_charge_seg['总时间'].min() if not cv_charge_seg.empty else 0
        ...
        cv_window_time = charge_window_seg['总时间'].max() - charge_window_seg[
            '总时间'].min() if not charge_window_seg.empty else 0
...
if __name__ == '__main__':
    ...
    root_dir_xlsx = r'/home/scuee_user06/myh/电池/data/xlsx/1'
    ...
    output_dir = r"/home/scuee_user06/myh/电池/data/selected_feature/statistic"
